...
class Mel:
    ...
    def update(self, screen, sprite_list, key, current_room):
        ...
        if self.catventurete.room == 7 and (abs(self.catventurete.current_coin_room[0].rect.x - self.rect.x) <= 400 or abs(self.catventurete.current_coin_room[0].rect.x - self.rect.x) <= 400):
            self.over = True
            self.darkness += 1
            self.true_darkness = self.darkness - 2000
